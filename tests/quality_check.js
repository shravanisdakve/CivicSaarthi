...
function checkLanguageArchitecture() {
  ...
  assert.ok(
    contextContent.includes("localStorage.getItem('civicsaarthi_language')"),
    'Context does not read from localStorage'
  );
  assert.ok(
    contextContent.includes("localStorage.setItem('civicsaarthi_language', newLang)"),
    'Context does not save to localStorage'
  );
...
function checkAntiPatterns() {
  ...
  components.forEach((comp) => {
    ...
    assert.strictEqual(
      hasManualListener,
      false,
      `Component ${comp} still has manual language listener!`
    );
...
function checkGroundingData() {
  ...
  assert.ok(
    assistantContent.includes('Grounded in official sources'),
    'Assistant UI missing grounding branding'
  );
...
function checkInputSafety() {
  ...
  assert.ok(
    content.includes('containsUnsafePersonalDataHint'),
    'containsUnsafePersonalDataHint missing'
  );
...
function checkAccessibilitySpecs() {
  ...
  assert.ok(
    ...
    'Reduced motion support missing in CSS'
  );
...
function checkGoogleServicesVisibility() {
  ...
  const qualityContent = fs.readFileSync(
    path.join(WORKSPACE_ROOT, 'src/pages/Quality.jsx'),
    'utf-8'
  );
  assert.ok(
    qualityContent.includes('Google Services Integration'),
    'Quality page missing Google Services section'
  );
  assert.ok(
    qualityContent.includes('Secret Manager'),
    'Quality page missing Secret Manager mention'
  );
  ...
  const assistantContent = fs.readFileSync(
    path.join(WORKSPACE_ROOT, 'src/pages/Assistant.jsx'),
    'utf-8'
  );
  assert.ok(
    assistantContent.includes('Gemini API Active'),
    'Assistant missing Gemini status badge'
  );

  const mapContent = fs.readFileSync(path.join(WORKSPACE_ROOT, 'src/pages/MapHelper.jsx'), 'utf-8');
  assert.ok(
    mapContent.includes('Google Maps Platform Integration'),
    'Map Helper missing Maps attribution'
  );

  const readmeContent = fs.readFileSync(path.join(WORKSPACE_ROOT, 'README.md'), 'utf-8');
  assert.ok(
    readmeContent.indexOf('## Google Services Integration') < 500,
    'README missing Google Services section near the top'
  );
