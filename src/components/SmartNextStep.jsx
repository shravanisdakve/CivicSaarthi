...
export default function SmartNextStep() {
  ...
  const steps = useMemo(() => getSmartRecommendation(personaId, completedCount), [personaId, completedCount]);
      ...
      <ul className="space-y-4 mb-8">
        {steps.map((step, idx) => (
          <li key={idx} className="flex items-start gap-3">
            <span className="material-symbols-outlined text-secondary mt-0.5 text-[20px]">
              check_circle
            </span>
