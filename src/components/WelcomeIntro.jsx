...
export default function WelcomeIntro({ isOpen, onClose }) {
  ...
  const handleNext = useCallback(() => setStep((s) => s + 1), [setStep]);
      ...
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="h-1.5 bg-slate-100 flex">
          {[1, 2, 3, 4].map((s) => (
            <div
              key={s}
              className={`flex-grow transition-all duration-500 ${s <= step ? 'bg-primary' : 'bg-transparent'}`}
            ></div>
          ))}
        </div>
        ...
        <div className="p-8 md:p-10 flex-grow relative">
          {step === 1 && (
            <div key="step1" className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              ...
              <p className="text-sm font-bold text-primary tracking-wide uppercase">
                Your neutral election-readiness companion.
              </p>
              <p className="text-sm text-slate-600 leading-relaxed">
                CivicSaarthi helps you understand the election process, timelines, voter steps, key
                terms, and official verification in a simple step-by-step way.
              </p>
              <Button
                onClick={handleNext}
                className="w-full py-4 text-base font-bold shadow-lg mt-4"
              >
                Next
              </Button>
            </div>
          ...
          {step === 2 && (
            <div key="step2" className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              <h2 className="text-2xl font-bold font-['Public_Sans'] text-slate-900">
                What you can do
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  ...
                  <p className="text-[10px] text-slate-500">
                    Get simple explanations for election terms.
                  </p>
                </div>
          ...
          {step === 3 && (
            ...
            >
              <h2 className="text-2xl font-bold font-['Public_Sans'] text-slate-900">
                Neutral and privacy-first
              </h2>
              <p className="text-sm text-slate-600 leading-relaxed max-w-xs mx-auto">
                CivicSaarthi does not support or oppose any party or candidate. It does not collect
                Aadhaar, voter ID, phone number, or political preferences.
              </p>
              <div className="flex flex-col items-center gap-3 pt-4">
                ...
                <Badge variant="default" className="px-4 py-2 text-xs">
                  Privacy-conscious
                </Badge>
              </div>
              <Button
                onClick={handleNext}
                className="w-full py-4 text-base font-bold shadow-lg mt-6"
              >
                Next
              </Button>
            </div>
          ...
          {step === 4 && (
            ...
            >
              ...
              <p className="text-sm text-slate-600 leading-relaxed">
                Learn the election process one step at a time, with simple explanations and links to
                the visual timeline.
              </p>
