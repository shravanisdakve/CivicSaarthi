...
export default function Quiz() {
  ...
  const handleSelect = useCallback((idx) => {
    ...
    setSelected(idx);
  }, [showExplanation, setSelected]); // Dependencies
  ...
  }, [selected, setShowExplanation, setCorrectCount, question?.correct]); // Dependencies
  ...
  }, [currentIdx, setCompleted, setCurrentIdx, setSelected, setShowExplanation]); // Dependencies
  ...
  const handleRestart = useCallback(() => {
    ...
    setCorrectCount(0);
    setCompleted(false);
  }, [setCurrentIdx, setSelected, setShowExplanation, setCorrectCount, setCompleted]); // Dependencies
  ...
  return (
    <div className="max-w-screen-md mx-auto px-6 py-12">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6 shadow-sm flex items-center justify-between">
        <span className="font-semibold text-primary text-sm font-['Public_Sans']">
      ...
      <Card className="p-6 md:p-8">
          ...
          {question.options.map((opt, idx) => {
            ...
            if (showExplanation) {
              if (idx === question.correct) {
                ...
                icon = (
                  <span className="material-symbols-outlined text-green-600 icon-fill shrink-0">
                    check_circle
                  </span>
                );
              } else if (isSelected) {
                ...
                icon = (
                  <span className="material-symbols-outlined text-red-600 icon-fill shrink-0">
                    cancel
                  </span>
                );
              } else {
              ...
              if (isSelected) {
                ...
                icon = (
                  <span className="material-symbols-outlined text-primary icon-fill shrink-0">
            ...
            return (
              ...
              >
                {icon}
                <span
                  className={`text-sm ${showExplanation && isSelected && !isCorrect ? 'text-red-900' : 'text-on-surface'}`}
                >
                  {opt}
                </span>
              </button>
        ...
        {showExplanation && (
          <div
            className={`p-4 rounded-xl mb-6 flex gap-3 ${isCorrect ? 'bg-green-50 text-green-900' : 'bg-red-50 text-red-900'}`}
          >
            <span className="material-symbols-outlined mt-0.5 icon-fill">
