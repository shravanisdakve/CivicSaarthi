...
export default function Timeline() {
  ...
  }, [activeIndex, setActiveId]); // Dependencies
  ...
  const getContextualAction = useCallback((stage) => {
    if (stage.id === 2) {
    ...
    if (stage.id === 7) {
      return {
        ...
        icon: 'location_on',
        action: () =>
          navigate('/assistant', { state: { question: 'How do I find my polling station?' } }),
      };
    }
    if (stage.id === 1) {
    ...
    <div className="max-w-screen-xl mx-auto px-6 md:px-8 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-on-surface-variant max-w-xl mx-auto text-sm">
          Understanding the electoral process from announcement to results. Follow the journey
          step-by-step.
        </p>
      </div>
      ...
      {currentPhase && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-card p-4 mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-primary bg-primary-fixed w-10 h-10 rounded-full flex items-center justify-center shrink-0">
              {currentPhase.icon}
            </span>
            <div>
              <p className="font-['Public_Sans'] font-semibold text-on-surface">
                Current Phase: {currentPhase.title}
              </p>
              <p className="text-xs text-on-surface-variant">
                Step {currentPhase.id} of {timelineStages.length} • {currentPhase.date}
              </p>
            </div>
          </div>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <h3 className="font-['Public_Sans'] font-semibold text-on-surface mb-1">
                      {stage.title}
                    </h3>
                ...
                <div className="bg-slate-800 h-40 flex items-end p-5 relative">
                  ...
                  <div className="relative z-10">
                    {activeStage.status === 'active' && (
                      <span className="text-[10px] font-bold bg-primary text-white px-2 py-0.5 rounded-sm uppercase tracking-wider mb-2 inline-block">
                        ACTIVE PHASE
                      </span>
                    )}
                    <h2 className="font-['Public_Sans'] font-bold text-white text-2xl">
                      {activeStage.title}
                    </h2>
                ...
                <div className="p-6">
                  <div className="flex flex-wrap gap-2 mb-6 border-b border-slate-100 pb-4">
                    ...
                    <button
                      onClick={() =>
                        setSelectedExplainer(
                          phaseExplainers.find((e) => e.phaseId === activeStage.id)
                        )
                      }
                    ...
                    >
                      ...
                      {understoodExplainers.includes(activeStage.id)
                        ? 'Review 30-sec Explainer'
                  ...
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                      Stage {activeStage.id} of {timelineStages.length}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        navigate('/assistant', {
                          state: {
                            question: `Explain the "${activeStage.title}" phase of Indian elections in detail.`,
                          },
                        })
                      }
                    >
