...
export default function Sources() {
  ...
  const handleReportMisinfo = () => {
    navigate('/assistant', { state: { question: 'I found some suspicious information about voting. Can you help me verify it against official rules?' } });
  };
    ...
    <div className="max-w-screen-xl mx-auto px-6 md:px-8 py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        ...
        <div className="shrink-0">
          <div className="inline-flex items-center gap-2 bg-green-50 border border-green-200 px-4 py-2 rounded-full text-sm font-semibold text-green-700">
            <span className="material-symbols-outlined text-green-600 icon-fill text-lg">verified</span>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {officialSources.map((source) => (
          <Card key={source.id} className="p-6 flex flex-col justify-between group" accent={source.accent}>
            <div>
              ...
              <p className="text-sm text-on-surface-variant mb-6 leading-relaxed min-h-[3rem]">{source.description}</p>
            </div>
            <div className="flex flex-col gap-2">
              ...
              >
                {source.action}
                <span className="material-symbols-outlined text-sm">{source.action === 'Visit Portal' ? 'open_in_new' : 'download'}</span>
              </a>
              <button 
      ...
      <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="relative z-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-8">
              {misinfoWarnings.map((warning, idx) => (
                <div key={idx} className="flex items-start gap-2 text-sm text-red-800 bg-white/50 p-3 rounded-lg border border-red-100">
                  <span className="material-symbols-outlined text-red-500 shrink-0 text-base">close</span>
                  {warning}
                </div>
              ))}
            </div>
            <Button variant="primary" className="bg-red-600 hover:bg-red-700 border-none" onClick={handleReportMisinfo}>
        ...
        <div className="bg-surface-container-low border border-slate-200 rounded-2xl p-6 md:p-8 flex flex-col">
          <h2 className="font-['Public_Sans'] text-lg font-bold text-on-surface mb-5">Quick Verification</h2>
          <div className="flex flex-col gap-3 flex-grow">
            {verificationChecklist.map((item, idx) => (
              <div key={idx} className="flex items-center gap-3 bg-white p-4 rounded-xl border border-slate-100 shadow-sm transition-transform hover:translate-x-1">
                <span className="material-symbols-outlined text-green-600 icon-fill text-lg shrink-0">check_circle</span>
                <span className="text-xs font-bold text-on-surface uppercase tracking-tight">{item}</span>
              </div>
            ))}
          </div>
          <p className="text-[10px] text-slate-400 mt-6 italic">Source: ECI Communication Guidelines</p>
        </div>
      </div>
    </div>
  );
